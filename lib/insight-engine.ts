...
export function generateFinancialScore(
  ...
  if (categoriesAboveAverage.length > 0) {
    score -= 6;
    const top = categoriesAboveAverage.sort((a, b) => b.vsAverageAmount - a.vsAverageAmount)[0];
    factors.push({
  ...
  if (categoriesBelowAverage.length > 0) {
    score += 6;
    const top = categoriesBelowAverage.sort((a, b) => a.vsAverageAmount - b.vsAverageAmount)[0];
    factors.push({
  ...
  if (increasedCategories.length > 0) {
    ...
    insights.push({
      ...
      description: `${top.category} er opp ${Math.abs(top.changePercent ?? 0)
        .toFixed(1)
        .replace(".", ",")} % fra forrige måned (${Math.abs(top.changeAmount).toLocaleString(
        "no-NO"
      )} kr mer).`,
      severity: "warning",
    });
